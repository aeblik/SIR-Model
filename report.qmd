...
The two-dimensional Laplace operator is defined as the sum of second derivatives in each coordinate direction, $\nabla^2 u = \partial_{xx}u + \partial_{yy}u.$ We discretise $\partial_{xx}u$ and $\partial_{yy}u$ using second-order central differences and sum them to obtain a discrete Laplacian on the grid. Resulting in a 5-point stencil, where every grid point is defined as: $$ \Delta^2u_{i,j} \approx \frac{u_{i+1,j}-2u_{i,j}+u_{i-1,j}}{\Delta x^2}+\frac{u_{i,j+1}-2u_{i,j}+u_{i,j-1}}{\Delta y^2} $$
