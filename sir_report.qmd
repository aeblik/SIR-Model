...
-   **PDE System**: Reaction-diffusion equations with diffusion for S and I, source term for R

-   **Numerical Method**: Finite difference (central differences for Laplacian), Forward Euler time integration

-   **Spatial Grid**: 101×101 points, 10×10 cm domain

-   **Boundary Conditions**: Zero-flux Neumann (closed system)

-   **Stability**: Time step constrained by diffusion limit (dt \< 0.5 × dt_max)

-   **Parameters**: β (transmission), γ (recovery), Ds/Di (diffusion coefficients)

-   **Initial Conditions**: Gaussian infected seed at center, uniform susceptible

-   **Scenarios**: 4 combinations of low/high β and γ values

-   **Outputs**: Spatial snapshots, integrated totals (S, I, R over time)

-   **Sensitivity Analysis**: Multi-trajectory comparison across parameter space

## Initial Conditions Setup

-   **Domain**: 10×10 cm, 101×101 grid points

-   **Infected (I)**: Gaussian distribution centered at (5, 5) cm

    -   Peak value: 0.02 individuals/cm²

    -   Standard deviation (σ): 0.5 cm

    -   Formula: I = 0.02 × exp(-((x-5)² + (y-5)²) / (2×0.5²))

-   **Recovered (R)**: Zero everywhere

-   **Susceptible (S)**: N₀ - I - R, where N₀ = 1.0 individuals/cm²

-   **Conservation**: S + I + R = 1.0 at all spatial points initially

This creates a localized infection "seed" at the center of the domain with the majority of the population susceptible.

## Time stepping routine

-   **Method**: Explicit Forward Euler (first-order)

-   **Time step**: Stability-constrained by diffusion limit

    -   dt_max = 1 / (2·D_max·(1/dx² + 1/dy²))

    -   dt = 0.5 × dt_max (safety factor)

-   **Algorithm per step** (`sir_step` function):

    1.  Apply Neumann boundary conditions to S and I

    2.  Calculate total population N = S + I + R

    3.  Compute reaction terms: infection = β·S·I/N, recovery = γ·I

    4.  Compute diffusion terms: ∇²S and ∇²I using finite differences
    ...
    6.  Clip negative values to zero

-   **Loop**: Iterates for nt steps (nt ≈ 73,000 for 365 days)

-   **Storage**: Snapshots saved every 50 steps
