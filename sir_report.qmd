...
The simulation was initiated by the run_simulation function, which combined all the model's components. This function initialised the spatial grids with the defined initial conditions and set the epidemiological parameters (β and γ) and the diffusion coefficients (D_s and D_i). Four different scenarios were simulated to compare disease dynamics under varying conditions. The function executed the time loop by repeatedly calling the `sir_step` function to advance the solution forward in time.
