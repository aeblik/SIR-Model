...
The two-dimensional Laplace operator is defined as the sum of second derivatives in each coordinate direction, \$ \nabla\^2 u = \partial*{xx}u +* \partial{yy}u. \$ We discretise $(\partial_{xx}u\)$ and $(\partial_{yy}u\)$ using second-order central differences and sum them to obtain a discrete Laplacian on the grid.
